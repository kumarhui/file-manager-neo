...
import org.fossify.filemanager.customtools.ui.CustomToolsSheet
...
import org.fossify.filemanager.customtools.ui.PdfPageExtractionDialog
...
class ItemsAdapter(
    ...
    override fun getActionMenuId() = R.menu.cab
        ...
        menu.apply {
            findItem(R.id.cab_decompress)?.isVisible = paths.any { it.isZipFile() }
            findItem(R.id.cab_confirm_selection)?.isVisible = isPickMultipleIntent
            findItem(R.id.cab_tool_custom_sheet)?.isVisible = selected.isNotEmpty()
        }
    }
        ...
        when (id) {
            ...
            R.id.cab_tool_custom_sheet -> handleToolCustomSheet()
        }
    }
    ...
    private fun deleteFiles() {
        ...
        activity.handleSAFDialog(SAFPath) { granted ->
            ...
            ensureBackgroundThread {
                ...
                private fun handleToolCustomSheet() {
        val paths = getSelectedFileDirItems().filter { !it.isDirectory }.map { it.path }
        if (paths.isEmpty()) return
        CustomToolsSheet.show(activity, paths)
        // Selection remains active instead of calling finishActMode()
    }

    private fun handleToolPrint() {
        val paths = getSelectedFileDirItems().filter { !it.isDirectory }.map { it.path }
        if (paths.isEmpty()) return
        NokoPrintHelper.print(activity, paths.first())
        finishActMode()
    }

    private fun handleToolPassportPhoto() {
        val path = getSelectedFileDirItems().firstOrNull()?.path ?: return
        PassportPhotoConfigDialog.show(activity, path)
        finishActMode()
    }

    private fun handleToolIdCard() {
        val path = getSelectedFileDirItems().firstOrNull()?.path ?: return
        CustomToolRunner.createIdCard(activity, path)
        finishActMode()
    }

    private fun handleToolConvertToPdf() {
        val path = getSelectedFileDirItems().firstOrNull()?.path ?: return
        CustomToolRunner.convertPdf(activity, path)
        finishActMode()
    }

    private fun handleToolPdfUnlocker() {
        val path = getSelectedFileDirItems().firstOrNull()?.path ?: return
        PdfUnlockerDialog.show(activity, Uri.fromFile(File(path)))
        finishActMode()
    }

    private fun handleToolConvertToImage() {
        val paths = getSelectedFileDirItems().map { it.path }
        if (paths.isNotEmpty()) {
            PdfPageExtractionDialog.show(activity, paths)
        }
        finishActMode()
    }
